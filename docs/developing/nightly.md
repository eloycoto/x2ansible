...
Rather than typing the `package` lines by hand, you can derive the full list of X2A plugin names directly from [`rhdh-plugins-latest.json`]({% link platform/plugin-compatibility.md %}), the same data file that drives the [Plugin Compatibility]({% link platform/plugin-compatibility.md %}) table. The plugin keys in that file always match the current set of X2A plugins that need to be shipped, so it's a reliable source for scripting the nightly `package` block.
